...
#    Nrs,Nls,regs,transients,spectral_radiuss,leaky_rates,input_scale
    ...
    # plt.legend()
    # plt.show()
    name='Ocean energy/Results/R_Hybrid_Mul'+stats_m+'NUMFEASedRVFLFSLR'+str(Nls[0])+station+year+'step'+str(step)+'s'+str(s)+'.pkl'
    f=open(name,'wb')
    ...
    for nnn in range(5,6):
        ...
        for year in ['2017','2018','2019'][:]:
            for station in ['46083h','46080h','46076h','46001h'][:]:
                ...
                # print(statspre.shape,statstpre.shape)
                statspre[-test_l:]=statstpre
                stats_len=statspre.shape[0]
                # print(data_.shape,statspre.shape)
                residuals=np.divide(data_[-stats_len:,0],statspre[-stats_len:,0])
                # ml_data['WVHT']=data_
                ...
                hyper_pd.to_csv('R_HyperMul'+year+station+str(step)+stats_m+'edRVFL.csv')
                train_idx=range(train_l+val_l)
                ...
                for s in np.arange(seeds):
                    ...
                    test_outputs_ea=np.multiply(test_outputs_res[:,0],statspre[-test_l:,0])
                ...
                # df.to_csv('Ocean energy/Results/R_Hybrid_Mul'+stats_m+'NUMFEASedRVFLFSLR'+str(Nls[0])+station+year+'step'+str(step)+'.csv')
                ...
                dfea.to_csv('New_run/R_Hybrid_Mul'+stats_m+'edRVFL'+str(Nls[0])+station+year+str(boat)+'step'+str(step))
...
# if __name__ == "__main__":
    ...
    #     resp=main()
                hyper_pd=pd.read_csv('R_HyperMul'+year+station+str(step)+stats_m+'edRVFL.csv')[['Nh','reg','iss','ratio']]
                idx=[]
                for i in range(5):
                ...
                p.to_csv('R_HyperMul'+year+station+str(step)+stats_m+'edRVFL.csv')    