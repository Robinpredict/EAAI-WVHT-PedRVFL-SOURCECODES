...
#    Nrs,Nls,regs,transients,spectral_radiuss,leaky_rates,input_scale
    ...
    name='Ocean energy/Results/NUMFEASedRVFLFSLR'+str(Nls[0])+station+year+'step'+str(step)+'s'+str(s)+'.pkl'
    f=open(name,'wb')
    ...
    return np.mean(outputs,axis=1).reshape(-1,1),num_feas#outputs.mean(axis=1).reshape(-1,1)
    ...
    for nnn in range(5,6):
        ...
        for year in ['2017','2018','2019']:
            for station in ['46083h','46080h','46076h','46001h']:
                ...
                # statspre=pd.read_csv('Ocean energy/Results/ARIMA_all'+station+year+'step'+str(step)+'.csv').values[:,1]
                # stats_len=statspre.shape[0]
                # print(data_.shape,statspre.shape)
                # residuals=data_[-stats_len:,0]-statspre[-stats_len:]
                ml_data['WVHT']=data_
                ...
                target_norm=target_scaler.transform(data_.reshape(-1,1))
                ...
                hyper_pd.to_csv('edRVFLFSLRPool'+year+station+str(step)+'.csv')
                train_idx=range(train_l+val_l)
                scaler.fit(ml_data.values[:-test_l])
                norm_data=scaler.transform(ml_data.values)
                target_scaler=preprocessing.MinMaxScaler() 
                target_scaler.fit(data_[:-test_l].reshape(-1,1))
                target_norm=target_scaler.transform(data_.reshape(-1,1))
                data.inputs,data.targets=format_data(norm_data,order,target_norm)
                    # print(ed_best_hypers)
                NUMFS=[]
                for s in np.arange(seeds):
                    np.random.seed(s)
                    test_outputs_norm_mea,num_feas=edRVFL_predict(ed_best_hypers,data,train_idx,test_idx,s)
                    NUMFS.append(num_feas)
                    # test_outputs_ed=scaler.inverse_transform(test_outputs_norm_med)
                    test_outputs_ea=target_scaler.inverse_transform(test_outputs_norm_mea)#+data_[-test_l-1:-1]
                    # print(test_outputs_res.shape)
                    # test_outputs_ea=test_outputs_res[:,0]+statspre[-test_l:]
                    ...
                    # plt.figure()
                    # plt.plot(actuals[-100:])
                    # plt.plot(test_outputs_ea[-100:])
                    # plt.show()
                ...
                # df.to_csv('Ocean energy/Results/NUMFEASedRVFLFSLR'+str(Nls[0])+station+year+'step'+str(step)+'.csv')
                test_p=np.concatenate(test_pres_ea,axis=0)
                # print(test_p.shape)
                dfea=pd.DataFrame(test_p)
                #D:\River flow forecasting\Results
                dfea.to_csv('New_run/edRVFLFSLRPool'+str(Nls[0])+station+year+str(boat)+'step'+str(step))
