    ...
    def computeLayerState(self,  layer,x_raw,inistate=None, DeepIP = 0):
        ...
        if layer==0:
            ...
            if self.act=='sigmoid':
                ...
                state=np.tanh(self.Win[layer][:,:-1].dot(x_raw))+np.tile(np.expand_dims(self.Win[layer][:,-1],1),[1,Ns])
            else:
            ...
            if self.act=='sigmoid':
                ...
                state=np.tanh(self.Win[layer][:,:-1].dot(x_cat))+np.tile(np.expand_dims(self.Win[layer][:,-1],1),[1,Ns])
            else:
